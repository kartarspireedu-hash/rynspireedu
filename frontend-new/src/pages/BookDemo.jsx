...
import { ArrowLeft, ArrowRight, CalendarIcon, CheckCircle2, Sparkles, Clock, Video } from "lucide-react";
...
const TZ = [
  ...
  "Asia/Kolkata", "Asia/Singapore", "Asia/Dubai", "Europe/London", "America/New_York", "America/Los_Angeles",
];
...
export default function BookDemo() {
  ...
  const [step, setStep] = useState(1); // 1=schedule, 2=form, 3=success
  ...
  const [form, setForm] = useState({
    ...
    student_class: "Grade 8", subject: "Mathematics",
    additional_notes: "",
  });
      ...
      <section className="container-x pt-10 pb-16">
        <div className="max-w-3xl mx-auto">
          ...
          <AnimatePresence mode="wait">
                ...
                <div className="mt-6 grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="sm:col-span-2">
                    ...
                    <Select value={tz} onValueChange={setTz}>
                      ...
                      <SelectContent>
                        {TZ.map((t) => <SelectItem key={t} value={t}>{t.replace("_", " ")}</SelectItem>)}
                      </SelectContent>
                    </Select>
                  </div>
                </div>
            ...
            {step === 2 && (
              <motion.form key="s2" onSubmit={submit} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.3 }} className="mt-10 rounded-3xl border border-border bg-card p-6 sm:p-8" data-testid="demo-form">
                ...
                <div className="mt-6 grid sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input id="name" required value={form.name} onChange={(e) => setField("name", e.target.value)} className="mt-1.5 rounded-xl" data-testid="demo-name" />
                  </div>
                  <div>
                ...
                {/* Summary */}
                <div className="mt-6 rounded-2xl bg-secondary/60 border border-border p-4 text-sm">
                  <p className="text-xs uppercase tracking-[0.2em] text-primary/80">Your demo</p>
                  <p className="mt-2"><strong>{fmtDate(date)}</strong> at <strong>{time}</strong> ({tz.replace("_", " ")}) · 25 minutes</p>
                </div>

                <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
                  <p className="text-xs text-muted-foreground">By continuing, you agree to be contacted at the details above.</p>
                  <Button type="submit" disabled={busy} className="pill-btn bg-primary text-primary-foreground hover:bg-accent hover:text-accent-foreground" data-testid="demo-submit-btn">
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="mt-10 rounded-3xl border border-accent/40 bg-gradient-to-br from-secondary via-background to-accent/10 p-8 sm:p-10 text-center" data-testid="demo-success">
                <div className="mx-auto h-16 w-16 rounded-full bg-accent/25 grid place-items-center">
                  <CheckCircle2 size={32} className="text-primary" />
                </div>
                <h2 className="mt-5 font-display text-3xl">You&apos;re all set, {form.name.split(" ")[0]}!</h2>
                <p className="mt-2 text-muted-foreground">A confirmation email is on its way to <strong>{form.email}</strong>. Our coordinator will call you within 12 hours on <strong>{form.phone}</strong>.</p>

                <div className="mt-6 mx-auto max-w-md rounded-2xl bg-background/70 backdrop-blur-sm border border-border p-5 text-left text-sm">
                  <div className="flex justify-between py-1"><span className="text-muted-foreground">Subject</span><span>{form.subject}</span></div>
