...
export default function BrandMark({ size = "md", withLogo = true, className = "" }) {
  ...
  const sizes = {
    xs: { logo: "h-8 w-8", text: "text-base", spark: 10 },
    sm: { logo: "h-10 w-10", text: "text-lg", spark: 12 },
    md: { logo: "h-12 w-12", text: "text-xl", spark: 12 },
    lg: { logo: "h-16 w-16", text: "text-3xl", spark: 16 },
    xl: { logo: "h-20 w-20", text: "text-4xl sm:text-5xl", spark: 22 },
  };
  const s = sizes[size] || sizes.md;

  return (
    <span className={`inline-flex items-center gap-1 ${className}`} data-testid="brand-mark">
      {withLogo && logoUrl && (
