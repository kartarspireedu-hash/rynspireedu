...
const STORAGE_KEY = "rse_whatsapp_btn_pos";
const BTN_SIZE = 56;
...
function defaultPos() {
  // Bottom-left by default, clear of the mobile sticky bar / desktop footer area.
  const isMobile = window.innerWidth < 640;
  return clamp({
    x: 16,
    y: window.innerHeight - BTN_SIZE - (isMobile ? 96 : 24),
  });
}
...
export default function FloatingWhatsApp() {
  ...
  // Re-clamp on resize/orientation change so it never gets stuck off-screen.
  useEffect(() => {
  ...
  const onPointerUp = useCallback(() => {
    ...
    if (d.moved) {
      ...
      // A genuine tap (no real movement) — open WhatsApp.
      window.open(
  ...
  return (
    <button
      ...
      className="fixed z-[60] h-14 w-14 rounded-full bg-[#25D366] shadow-[0_10px_30px_rgba(37,211,102,0.45)] grid place-items-center hover:scale-105 active:scale-95 transition-transform cursor-grab active:cursor-grabbing select-none"
    >
